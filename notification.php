...
try {
    ...
    $stmt = $pdo->prepare("SELECT date, time, lora_noseries, profile_id, status FROM notification");
    $stmt->execute();
...
<head>
    ...
    <title>LORETRACK - Notification</title>
    ...
    <div class="grid grid-cols-4 gap-6">
        ...
        <div class="bg-blue-600 text-white p-4 rounded-lg h-screen">
            ...
            <nav>
                ...
                <a href="notification.php" class="block py-2 px-2 bg-blue-900 hover:bg-blue-700 rounded-lg">Status</a>
                ...
                <table class="min-w-full table-auto">
                    ...
                    <tbody>
                        <?php foreach ($notifications as $notification): ?>
                            <tr>
                                <td class="px-4 py-2 border"><?= htmlspecialchars($notification['date']) ?></td>
                                <td class="px-4 py-2 border"><?= htmlspecialchars($notification['time']) ?></td>
                                <td class="px-4 py-2 border"><?= htmlspecialchars($notification['lora_noseries']) ?></td>
                                <td class="px-4 py-2 border"><?= htmlspecialchars($notification['profile_id']) ?></td>
                                <td class="px-4 py-2 border"><?= htmlspecialchars($notification['status']) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div class="fixed inset-x-0 bottom-0 p-4">
                <div class="max-w-auto mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex justify-end mt-4">
                        <button onclick="window.location.href='addstatus.php'" class="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-700">
                            Add Status
                        </button>
                    </div>
                </div>
